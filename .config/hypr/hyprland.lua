...
local float_w = math.floor(wide_w / 2)
local float_h = wide_h - 72
...
-- Values are derived from wide_mode.
hl.bind("SHIFT + ALT + 2", function()
	hl.dispatch(hl.dsp.window.float({ action = "toggle" }))
	hl.dispatch(hl.dsp.window.resize({ x = float_w, y = float_h, relative = false }))
