...
let
  ...
  loadFromTOML = path: builtins.fromTOML (builtins.readFile path);
...
in
{
  inherit loadFromTOML getNixOSHosts getDarwinHosts getHomeManagerHosts hasTag isFormFactor;
  ...
  mkIfDesktop = config: host: lib.mkIf (host.desktop or false) config;
}
